...
import {
    ...
    Plus, ChevronRight, ChevronLeft, Star,
    Pointer, RefreshCw
} from 'lucide-react';
...
const ClientListOffres = () => {
    ...
    const fetchOffres = async (page) => {
        ...
        try {
            ...
            setOffres(prev => page === 1 ? newData : [...prev, ...newData]);

            if (newData.length === 0 || (response.data.meta && page >= response.data.meta.last_page)) {
                setHasMore(false);
            }
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case 'ouvert':
                return { label: 'Ouvert', color: 'bg-green-500', lightColor: 'bg-green-50 text-green-700 border-green-200', icon: CheckCircle2 };
            case 'pending':
                return { label: 'En attente', color: 'bg-amber-500', lightColor: 'bg-amber-50 text-amber-700 border-amber-200', icon: Timer };
            case 'completed':
                return { label: 'Terminée', color: 'bg-gray-500', lightColor: 'bg-gray-100 text-gray-700 border-gray-200', icon: Briefcase };
            default:
                return { label: status || 'Inconnu', color: 'bg-gray-500', lightColor: 'bg-gray-100 text-gray-700 border-gray-200', icon: AlertCircle };
        }
    };
    ...
    const getUrgencyConfig = (urgency) => {
        const configs = {
            ...
            faible: { label: 'Faible', color: 'bg-gray-400 text-white' },  // nouveau statut
        };
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                            {offres.map((job) => {
                                ...
                                const urgency = getUrgencyConfig(job.urgence);
                                        ...
                                        <div className="p-4 space-y-3">
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                                                    ...
                                                    <span className="truncate">{job.ville}, {job.adresse}</span>
                                                </div>
                                                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                                                    <Calendar className="w-3.5 h-3.5 text-gray-400" />
                                                    <span>{new Date(job.date_preferee).toLocaleDateString('fr-FR')}</span>
                                                </div>
                                                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                                                    <DollarSign className="w-3.5 h-3.5 text-gray-400" />
                                                    <span>{job.budget} DH</span>
                                                </div>
                                                <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
                                                    <Pointer className="w-3.5 h-3.5 text-gray-400" />
                                                    <span>{job.nombre_propositions} propositions</span>
                                                </div>
                                            </div>
